...
@app.get("/show-table")
async def read_root():
    """Show all of the rows in test_table."""
...
@app.get("/nowait")
async def nowait():
    """Using NOWAIT seems to stop the race condition but we need to handle the exception."""
    async with database.connection() as connection:
        async with connection.transaction() as t:
            try:
                results = await database.fetch_all(text("select id from test_table for update nowait"))

                # It's fine to SELECT ... NOWAIT more than once in the same transaction
                await database.fetch_all(text("select id from test_table for update nowait"))

                await asyncio.sleep(3)
                max_id = max([x["id"] for x in results])
                await database.execute(text("insert into test_table values ({}, {})".format(max_id+1, 0)))
...
async def with_for_update():
    ...
    async with database.connection() as connection:
        async with connection.transaction() as t:
            results = await database.fetch_all(text("select id from test_table for update"))
            await asyncio.sleep(3)
            max_id = max([x["id"] for x in results])
            await database.execute(text("insert into test_table values ({}, {})".format(max_id+1, 0)))
...
async def explicit_lock():
    """Using LOCK seems to stop the race condition but we need to avoid deadlock."""
    async with database.connection() as connection:
        async with connection.transaction() as t:
